...
class PagesController: UIViewController {
    ...
    func setup() {
        let usePageIndicator = controllers.count > 1  && Config.PageIndicator.isEnable
        if usePageIndicator {
    ...
    public func didChangeSlectedIndex(_ index: Int) {
        if self.selectedIndex != index && index < self.controllers.count && index >= 0 {
...
extension PagesController: PageIndicatorDelegate {
    ...
    func pageIndicator(_ pageIndicator: PageIndicator, didSelect index: Int) {
        self.didChangeSlectedIndex(index)
    }
}
